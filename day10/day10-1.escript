...
main(_) ->
    Joltages = read_input("p10.txt"),
    Chain = find_chain(0, Joltages, []),
    {OneDiff, _TwoDiff, ThreeDiff} = sum_chain(Chain),
    io:format("chain diffs: ~p~n", [OneDiff * ThreeDiff]).

sum_chain(Chain) ->
    lists:foldl(fun tally_diff/2, {0, 0, 0}, Chain).

tally_diff({1, _}, {One, Two, Three}) ->
    {One+1, Two, Three};
tally_diff({2, _}, {One, Two, Three}) ->
    {One, Two+1, Three};
tally_diff({3, _}, {One, Two, Three}) ->
    {One, Two, Three+1}.

find_chain(_CurrentJoltage, [], Chain) -> Chain;
find_chain(CurrentJoltage, Joltages, Chain) ->
    Diffs = [{Diff, Jolt} || Jolt <- Joltages,
                            Diff <- [Jolt-CurrentJoltage],
                            Diff > 0 andalso Diff < 4 % 1, 2, 3 diff
            ],
    [{Diff, Jolt} | _] = lists:keysort(1, Diffs),
    find_chain(Jolt, lists:delete(Jolt, Joltages), [{Diff, Jolt} | Chain]).

read_input(File) ->
    {'ok', Lines} = file:read_file(File),
    {Max, Joltages} = lists:foldl(fun to_joltage/2, {0, []}, binary:split(Lines, <<"\n">>, ['global'])),
    lists:sort([Max+3 | Joltages]). %% add device's bulit-in adapter

to_joltage(<<>>, Acc) -> Acc;
to_joltage(<<Line/binary>>, {Max, Joltages}) ->
    to_joltage(binary_to_integer(Line, 10), {Max, Joltages});
to_joltage(Jolt, {Max, Joltages}) when Jolt > Max ->
    {Jolt, [Jolt | Joltages]};
to_joltage(Jolt, {Max, Joltages}) ->
    {Max, [Jolt | Joltages]}.
