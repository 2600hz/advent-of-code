...
-define(TIME(Op)
       ,Start = erlang:monotonic_time(),
        ...
        Elapsed > 100_000 andalso io:format("~b ~p() took ~p~n", [?LINE, OpFun, Elapsed])
       ).
...
part1(Monkeys) ->
    {_LCM, Ms} = exec_rounds(init_relief(Monkeys, 3), ?ROUNDS),
    print_monkeys(Ms),
    MBs = maps:fold(fun monkey_business/3, [], Ms),
    [I1, I2 | _] = lists:reverse(lists:sort(MBs)),
    io:format("p1 monkey business: ~p~n", [I1 * I2]).

part2(Monkeys) ->
    {_LCM, Ms} = exec_rounds(init_relief(Monkeys, 1), ?ROUNDS),
    print_monkeys(Ms),
    MBs = maps:fold(fun monkey_business/3, [], Ms),
    [I1, I2 | _] = lists:reverse(lists:sort(MBs)),
    io:format("p2 monkey business: ~p~n", [I1 * I2]).

monkey_business(_No, #{inspections := I}, Is) ->
    [I | Is].

print_monkeys(Monkeys) ->
    maps:map(fun print_monkey/2, Monkeys).

print_monkey(No, #{inspections := Is}) ->
    io:format("  ~p inspected ~p~n", [No, Is]).

exec_round({LCM, Monkeys}, MonkeyNos) ->
    lists:foldl(fun exec_turn/2
               ,{LCM, Monkeys}
               ,MonkeyNos
               ).

exec_turn(MonkeyNo, {LCM, Monkeys}) ->
    #{items := Items
     ,inspections := Is
     }=Monkey
        = maps:get(MonkeyNo, Monkeys),
    {_, NewMs} = fold_items(LCM, {Monkey, Monkeys}, Items),
    {LCM
    ,NewMs#{MonkeyNo => Monkey#{inspections => Is + length(Items), items => []}}
    }.

fold_items(_LCM, Acc, []) -> Acc;
fold_items(LCM, Acc, [Item | Items]) ->
    fold_items(LCM, inspect_and_throw(LCM, Item, Acc), Items).

inspect_and_throw(LCM
                 ,ItemWorry
                 ,{#{test := DivBy
                    ,true_clause := T
                    ,false_clause := F
    ...
    ?TIME(Worry = OpFun(ItemWorry) div Relief),

    NewMs = case Worry rem DivBy of
                0 ->
                    throw_to(Worry rem LCM, T, Monkeys);
                _Rem ->
                    throw_to(Worry rem LCM, F, Monkeys)
            end,

    {Monkey, NewMs}.

throw_to(Worry, MonkeyNo, Monkeys) ->
    #{items := Items}=Monkey = maps:get(MonkeyNo, Monkeys),
    Monkeys#{MonkeyNo => Monkey#{items => [Worry | Items]}}.

exec_rounds({LCM, Monkeys}, Rounds) ->
    exec_rounds({LCM, Monkeys}, lists:sort(maps:keys(Monkeys)), Rounds, 1).

exec_rounds({LCM, Monkeys}, Nos, Rounds, Rounds) ->
    exec_round({LCM, Monkeys}, Nos);
exec_rounds({LCM, Monkeys}, Nos, Rounds, Round) ->
    {LCM, Ms} = exec_round({LCM, Monkeys}, Nos),
    exec_rounds({LCM, Ms}, Nos, Rounds, Round+1).

init_relief(Monkeys, Relief) ->
    Ms = maps:map(fun(_No, M) -> M#{relief => Relief} end, Monkeys),
    {maps:fold(fun lcm/3, 1, Ms), Ms}.

lcm(_K, #{test := DivBy}, LCM) ->
    LCM * DivBy.

input() ->
    Notes = binary:split(input:input(<<?MODULE_STRING>>), <<$\n>>, ['global', 'trim']),
    parse_notes(Notes).

parse_notes(Notes) ->
    parse_notes(Notes, {0, #{0 => #{no => 0, inspections => 0}}}).

parse_notes([], {_MonkeyNo, Monkeys}) -> Monkeys;
parse_notes([Note | Notes], {MonkeyNo, Monkeys}) ->
    {NewNo, NewMs} = parse_note(Note, {MonkeyNo, Monkeys}),
    parse_notes(Notes, {NewNo, NewMs}).

parse_note(<<>>, {No, Monkeys}) ->
    NewNo = No+1,
    {NewNo, maps:put(NewNo, #{no => NewNo, inspections => 0}, Monkeys)};
parse_note(<<"Monkey ", _/binary>>, {MonkeyNo, Monkeys}) ->
    {MonkeyNo, Monkeys};
parse_note(<<"  Starting items: ", ItemsBin/binary>>
          ,{No, Monkeys}
          ) ->
    Monkey = maps:get(No, Monkeys),
    Items = [binary_to_integer(Item, 10)
             || Item <- binary:split(ItemsBin, <<", ">>, ['global', 'trim'])
            ],
    {No, Monkeys#{No => Monkey#{items => lists:reverse(Items)}}};
parse_note(<<"  Operation: new = ", Op/binary>>, {No, Monkeys}) ->
    Monkey = maps:get(No, Monkeys),
    OpFun = parse_operation(Op),
    {No, Monkeys#{No => Monkey#{op => OpFun}}};
parse_note(<<"  Test: divisible by ", Test/binary>>, {No, Monkeys}) ->
    Monkey = maps:get(No, Monkeys),
    DivBy = binary_to_integer(Test, 10),
    {No, Monkeys#{No => Monkey#{test => DivBy}}};
parse_note(<<"    If true: throw to monkey ", TrueClause/binary>>, {No, Monkeys}) ->
    Monkey = maps:get(No, Monkeys),
    {No, Monkeys#{No => Monkey#{true_clause => binary_to_integer(TrueClause, 10)}}};
parse_note(<<"    If false: throw to monkey ", FalseClause/binary>>, {No, Monkeys}) ->
    Monkey = maps:get(No, Monkeys),
    {No, Monkeys#{No => Monkey#{false_clause => binary_to_integer(FalseClause, 10)}}}.

parse_operation(Operation) ->
    {'match', Result} = re:run(Operation
                              ,<<"([\\d]+|old) ([*|+|-|/]) ([\\d]+|old)">>
                              ,[{capture, all_but_first, binary}]
                              ),
    [LHS, Op, RHS] = [parse_op(O) || O <- Result],
    build_op_fun(LHS, Op, RHS).

parse_op(<<"old">>) -> 'old';
parse_op(<<"*">>) -> fun erlang:'*'/2;
parse_op(<<"+">>) -> fun erlang:'+'/2;
parse_op(Number) -> binary_to_integer(Number, 10).

build_op_fun('old', Op, 'old') ->
    fun(Old) -> Op(Old, Old) end;
build_op_fun('old', Op, RHS) ->
    fun(Old) -> Op(Old, RHS) end;
build_op_fun(LHS, Op, 'old') ->
    fun(Old) -> Op(LHS, Old) end;
build_op_fun(LHS, Op, RHS) ->
    fun(_Old) -> Op(LHS, RHS) end.
