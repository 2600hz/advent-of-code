...
part1(Rucksacks) ->
    Total = lists:sum([matching_item_priority(C1, C2) || {_Is, C1, C2} <- Rucksacks]),
    io:format("total of priorities: ~p~n", [Total]).

part2(Rucksacks) ->
    Total = group_rucksacks(Rucksacks, 0),
    io:format("total of group priorities: ~p~n", [Total]).

group_rucksacks([], Total) -> Total;
group_rucksacks([{R1, _, _}, {R2, _, _}, {R3, _, _} | Rucksacks], Total) ->
    [Common] = find_common([R1, R2, R3]),
    group_rucksacks(Rucksacks, priority(Common) + Total).

find_common([R | Rs]) ->
    find_common(Rs, R).

find_common([], Common) -> lists:usort(Common);
find_common([R | Rs], Common) ->
    find_common(Rs, keep_duplicates(lists:sort(R), lists:sort(Common), [])).

keep_duplicates([], _, Acc) -> Acc;
keep_duplicates(_, [], Acc) -> Acc;
keep_duplicates([X | Xs], [X | Cs], Acc) ->
    keep_duplicates(Xs, Cs, [X | Acc]);
keep_duplicates([X | Xs], [C | Cs], Acc) when X < C ->
    keep_duplicates(Xs, [C | Cs], Acc);
keep_duplicates(Xs, [_ | Cs], Acc) ->
    keep_duplicates(Xs, Cs, Acc).

matching_item_priority([{X, P} | _], [{X, P} | _]) -> P;
matching_item_priority([{X, _Xp} | Xs], [{Y, Yp} | Ys]) when X < Y ->
    matching_item_priority(Xs, [{Y, Yp} | Ys]);
matching_item_priority([{X, Xp} | Xs], [{Y, _Yp} | Ys]) when X > Y ->
    matching_item_priority([{X, Xp} | Xs], Ys).

input() ->
    Bin = input:input(<<?MODULE_STRING>>),
    RucksackItems = binary:split(Bin, <<$\n>>, ['global', 'trim']),
    [compartmentalize(Items) || Items <- RucksackItems].

compartmentalize(Items) ->
    Mid = byte_size(Items) div 2,
    <<Compartment1:Mid/binary, Compartment2:Mid/binary>> = Items,
    {[X || <<X>> <= Items]
    ,lists:keysort(1, [{X, priority(X)} || <<X>> <= Compartment1])
    ,lists:keysort(1, [{X, priority(X)} || <<X>> <= Compartment2])
    }.

priority(Lowercase) when Lowercase >= $a, Lowercase =< $z ->
    Lowercase - $a + 1;
priority(Uppercase) when Uppercase >= $A, Uppercase =< $Z ->
    Uppercase - $A + 27.
