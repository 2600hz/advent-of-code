...
part1([_|_]=Inputs) ->
    lists:foreach(fun part1/1, Inputs);
part1(<<Input/binary>>) ->
    Offset = find_start_of_packet_offset(Input),
    io:format("start of packet after ~p chars~n", [Offset]).
...
part2([_|_]=Inputs) ->
    lists:foreach(fun part2/1, Inputs);
part2(Input) ->
    Offset = find_start_of_message_offset(Input),
    io:format("start of message after ~p chars~n", [Offset]).
