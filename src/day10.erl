...
-record(instruction, {op, cycles, value}).
-record(cpu, {cycle, registers = #{}}).
...
part1([#instruction{} | _]=Instructions) ->
    CPU = lists:foldl(fun exec/2
                     ,{#cpu{cycle=1, registers = #{x => 1}}
                      ,{20, 0}
                      }
                     ,Instructions
                     ),
    io:format("exec: ~p~n", [CPU]).

exec(#instruction{op='noop', cycles=Hz}
    ,{#cpu{cycle=C, registers=#{x := XReg}}=CPU
     ,{TargetCycle, SignalStrength}
     }
    ) ->
    NewCycle = C+Hz,
    {CPU#cpu{cycle=NewCycle}
    ,track_signal(TargetCycle, SignalStrength, NewCycle, XReg)
    };
exec(#instruction{op='addx', cycles=Hz, value=V}
    ,{#cpu{cycle=C, registers=#{x := XReg}=Rs}=CPU
     ,{TargetCycle, SignalStrength}
     }
    ) ->
    NewCycle = C+Hz,
    {CPU#cpu{cycle=NewCycle, registers=Rs#{x => XReg + V} }
    ,track_signal(TargetCycle, SignalStrength, NewCycle, XReg)
    }.

track_signal(TargetCycle, SignalStrength, NewCycle, XReg)
  when NewCycle > TargetCycle ->
    {TargetCycle+40, (TargetCycle * XReg) + SignalStrength};
track_signal(TargetCycle, SignalStrength, _NewCycle, _XReg) ->
    {TargetCycle, SignalStrength}.

part2([#instruction{} | _]=Instructions) ->
    CPU = #cpu{cycle=1, registers = #{x => 1}},

    {_UpCPU, BackwardsCRT} = lists:foldl(fun exec_crt/2
                                        ,{CPU, []}
                                        ,Instructions
                                        ),
    CRT = lists:reverse(BackwardsCRT),
    print_crt(CRT).

print_crt([]) -> 'ok';
print_crt(CRT) ->
    {Row, Rest} = lists:split(40, CRT),
    io:format("~s~n", [Row]),
    print_crt(Rest).

exec_crt(#instruction{op='noop', cycles=Hz}
        ,{#cpu{cycle=C, registers=#{x := XReg}}=CPU
         ,CRT
         }
        ) ->
    NewCycle = C+Hz,
    {CPU#cpu{cycle=NewCycle}
    ,update_crt(CRT, XReg, C, NewCycle)
    };
exec_crt(#instruction{op='addx', cycles=Hz, value=V}
        ,{#cpu{cycle=C, registers=#{x := XReg}=Rs}=CPU
         ,CRT
         }
        ) ->
...
update_crt(CRT, _XReg, ToCycle, ToCycle) -> CRT;
update_crt(CRT, XReg, FromCycle, ToCycle) ->
    update_crt(update_pixel(CRT, XReg, pixel_pos(FromCycle))
              ,XReg
              ,FromCycle+1
              ,ToCycle
              ).
...
update_pixel(CRT, XReg, PixelPos)
  when XReg-1 =:= PixelPos;
       XReg =:= PixelPos;
       XReg+1 =:= PixelPos
       ->
    [?PIXEL_LIGHT | CRT];
update_pixel(CRT, _XReg, _PixelPos) ->
    [?PIXEL_DARK | CRT].

input() ->
    [parse_instruction(Instruction)
     || Instruction <- binary:split(input:input(<<?MODULE_STRING>>), <<$\n>>, ['global', 'trim'])
    ].

parse_instruction(<<"addx ", V/binary>>) ->
    #instruction{op='addx', cycles=2, value=binary_to_integer(V)};
parse_instruction(<<"noop">>) ->
    #instruction{op='noop', cycles=1}.
