...
import { solveFuelForMass, normalizeData } from './helpers.js';
...
// :: [Number] -> [Number]
const solveFuelForEachMass = a => a.map(b => {
  let mass = b;
  let total = 0;

  do {
    mass = solveFuelForMass(mass);
    total += mass;
  } while (solveFuelForMass(mass) > 0);
...
fuelCounterUpper(import.meta.url);
