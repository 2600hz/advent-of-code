...
part2(Inventory) ->
    ...
    [{C1, E1}, {C2, E2}, {C3, E3} | _] = lists:reverse(lists:keysort(1, Counts)),
    io:format("elves ~p ~p ~p carry ~p ~p ~p: ~p~n"
             ,[E1, E2, E3, C1, C2, C3, C1+C2+C3]
             ).

most_carried(Elf, CalorieList, {E, C}) ->
    case lists:sum(CalorieList) of
        Total when Total > C ->
            {Elf, Total};
        _ -> {E, C}
    end.
...
calories_carried(<<>>, {ElfNo, Inventory}) ->
    {ElfNo+1, Inventory};
calories_carried(CalorieBin, {ElfNo, Inventory}) ->
    Calories = binary_to_integer(CalorieBin, 10),
    Carried = maps:get(ElfNo, Inventory, []),
    {ElfNo, Inventory#{ElfNo => [Calories | Carried]}}.
