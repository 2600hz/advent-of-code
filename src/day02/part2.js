...
// :: Number -> ([a] -> Promise([a]))
const pairFinderForOutput = output => intcode => new Promise(async (resolve, reject) => {
  for (let noun of Array(100).keys()) {
    for (let verb of Array(100).keys()) {
      const result = await produceOutputForPair({ intcode, pair: [ noun, verb ] });
      if (result === output) return resolve([noun, verb]);
    }
  }
  return reject(new Error(`No pair found for output ${output}`));
});

// :: [a] -> [a]
const findPair = pairFinderForOutput(19690720);
...
computer(import.meta.url).catch(console.log);
