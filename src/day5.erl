...
-define(MOVEMENT(N, From, To), {N, From, To}).
...
part1({Stacks, Instructions}) ->
    FinalStacks = lists:foldl(fun run_9000_instruction/2, Stacks, Instructions),
    Message = lists:foldr(fun top_crate/2, [], lists:keysort(1, maps:to_list(FinalStacks))),
    io:format("message: ~s~n", [Message]).

run_9000_instruction(?MOVEMENT(N, From, To), Stacks) ->
    FromStack = maps:get(From, Stacks),
    ToStack = maps:get(To, Stacks),
    {NewFrom, NewTo} = move_9000(N, FromStack, ToStack),
    Stacks#{From => NewFrom, To => NewTo}.

move_9000(0, FromStack, ToStack) -> {FromStack, ToStack};
move_9000(N, [Crate | FromStack], ToStack) ->
    move_9000(N-1, FromStack, [Crate | ToStack]).

top_crate({_StackNo, [TopCrate | _]}, Message) ->
    [TopCrate | Message].


part2({Stacks, Instructions}) ->
    FinalStacks = lists:foldl(fun run_9001_instruction/2, Stacks, Instructions),
    Message = lists:foldr(fun top_crate/2, [], lists:keysort(1, maps:to_list(FinalStacks))),
    io:format("message: ~s~n", [Message]).

run_9001_instruction(?MOVEMENT(N, From, To), Stacks) ->
    FromStack = maps:get(From, Stacks),
    ToStack = maps:get(To, Stacks),
    {NewFrom, NewTo} = move_9001(N, FromStack, ToStack),
    Stacks#{From => NewFrom, To => NewTo}.

move_9001(N, FromStack, ToStack) ->
    {ToMove, NewFrom} = lists:split(N, FromStack),
    {NewFrom, ToMove ++ ToStack}.

input() ->
    Bin = input:input(<<?MODULE_STRING>>),
    parse_stacks(binary:split(Bin, <<$\n>>, ['global', 'trim'])).

parse_stacks(Lines) ->
    parse_stacks(Lines, #{}).

parse_stacks([<<" 1", _/binary>>, <<>> | Lines], Stacks) ->
    parse_instructions(Lines, Stacks);
parse_stacks([Line | Lines], Stacks) ->
    parse_stacks(Lines, parse_stack(Line, Stacks, 1)).

parse_stack(<<>>, Stacks, _StackNo) -> Stacks;
parse_stack(<<"    ", Rest/binary>>, Stacks, StackNo) ->
    parse_stack(Rest, Stacks, StackNo+1);
parse_stack(<<"[", N:1/binary, "]">>, Stacks, StackNo) ->
    Containers = maps:get(StackNo, Stacks, []),
    Stacks#{StackNo => [N | Containers]};
parse_stack(<<"[", N:1/binary, "] ", Rest/binary>>, Stacks, StackNo) ->
    Containers = maps:get(StackNo, Stacks, []),
    parse_stack(Rest, Stacks#{StackNo => [N | Containers]}, StackNo+1).

parse_instructions(Lines, Stacks) ->
    Instructions = lists:foldl(fun parse_instruction/2, [], Lines),
    {maps:map(fun reverse_stack/2, Stacks), lists:reverse(Instructions)}.

reverse_stack(_StackNo, Stack) -> lists:reverse(Stack).

parse_instruction(Line, Instructions) ->
    {'match', [_N, _From, _To]=Instruction} =
        re:run(Line, <<"move ([\\d]+) from (\\d) to (\\d)">>, [{capture, all_but_first, binary}]),
    [list_to_tuple([binary_to_integer(X, 10) || X <- Instruction]) | Instructions].
