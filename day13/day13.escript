...
%% 1 falls out. It's a large sheet of transparent paper! The
%% transparent paper is marked with random dots and includes
...
fold_left(Dots, Position) ->
    {NewDots, Position} = maps:fold(fun fold_left_position/3, {Dots, Position}, Dots),
    NewDots.

fold_left_position({X, Y}, _V, {Dots, XPosition}) when XPosition < X ->
    {Dots#{{XPosition - (X - XPosition), Y} => $#
          ,{X, Y} => $.
          }
    ,XPosition
    };
fold_left_position(_XY, _V, Acc) -> Acc.

fold_up(Dots, Position) ->
    {NewDots, Position} = maps:fold(fun fold_up_position/3, {Dots, Position}, Dots),
    NewDots.

fold_up_position({X, Y}, $#, {Dots, YPosition}) when YPosition < Y ->
    {Dots#{{X, YPosition - (Y - YPosition)} => $#
          ,{X, Y} => $.
          }
    ,YPosition
    };
...
read_input(File) ->
    ...
    read_dots(binary:split(Lines, <<"\n">>, ['global']), #{}).

read_dots([<<>> | Lines], Dots) ->
    {Dots, read_instructions(Lines, [])};
read_dots([DotBin | Lines], Dots) ->
    [X, Y] = [binary_to_integer(Dot, 10) || Dot <- binary:split(DotBin, <<",">>)],
    read_dots(Lines, Dots#{{X, Y} => $#}).

read_instructions([], Folds) ->
    lists:reverse(Folds);
read_instructions([<<>> | Lines], Folds) ->
    read_instructions(Lines, Folds);
read_instructions([<<"fold along ", Axis:8, "=", PosBin/binary>> | Lines], Folds) ->
    read_instructions(Lines, [{Axis, binary_to_integer(PosBin, 10)} | Folds]).

pp_dots(Map) ->
    {MaxX, MaxY} = max_xy(Map),
    pp_dots(Map, {MaxX, MaxY}, {0, 0}).

pp_dots(Map, {MaxX, MaxY}, {MaxX, MaxY}) ->
    io:format("~s~n", [[maps:get({MaxX, MaxY}, Map, $.)]]);
pp_dots(Map, {MaxX, MaxY}, {X, Y}) when X > MaxX ->
    io:format("~n"),
    pp_dots(Map, {MaxX, MaxY}, {0, Y+1});
pp_dots(Map, {MaxX, MaxY}, {X, Y}) ->
    io:format("~s", [[maps:get({X, Y}, Map, $.)]]),
    pp_dots(Map, {MaxX, MaxY}, {X+1, Y}).

max_xy(Map) ->
    XYs = [{X, Y} || {{X, Y}, $#} <- maps:to_list(Map)],
    {Xs, Ys} = lists:unzip(XYs),
    {lists:max(Xs), lists:max(Ys)}.

count_dots(Map) ->
    lists:sum([1 || {_, $#} <- maps:to_list(Map)]).
