...
main(_) ->
    {Depart, BusIds} = read_input("p13.txt"),
    [{Diff, BusId} | _]= lists:keysort(1, [{BusId - (Depart rem BusId), BusId} || BusId <- BusIds]),
    io:format("bus ~p leaves in ~p: ~p~n", [BusId, Diff, BusId * Diff]).

read_input(File) ->
    {'ok', Lines} = file:read_file(File),
    [Depart, Buses] = binary:split(Lines, <<"\n">>, ['global', 'trim']),
    {binary_to_integer(Depart, 10)
    ,[binary_to_integer(BusId) || BusId <- binary:split(Buses, <<",">>, ['global', 'trim']),
                                 BusId =/= <<"x">>
     ]
    }.
