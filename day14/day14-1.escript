...
main(_) ->
    ...
    Sum = maps:fold(fun(_Loc, <<V:36/integer>>, Acc) -> V + Acc end, 0, Memory),
    io:format("sum: ~p~n", [Sum]).

run_instruction({'mem', Location, Value}, {Mask, Program}) ->
    {Mask, Program#{Location => mem(Value, Mask)}};
run_instruction({'mask', Mask}, {_, Program}) ->
    {to_bin_bits(Mask), Program}.

mem(Value, Mask) ->
    mask_value(to_bits(<<Value:36>>), Mask, []).

mask_value([], [], VRev) ->
    from_bits(lists:reverse(VRev));
mask_value([V | Vs], [$X | Ms], VRev) ->
    mask_value(Vs, Ms, [V | VRev]);
mask_value([_ | Vs], [M | Ms], VRev) ->
    mask_value(Vs, Ms, [M-$0 | VRev]).
