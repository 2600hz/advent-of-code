...
p14_1({Template, Insertions}) ->
    ...
    io:format("~p(~s) - ~p(~s) = ~p~n", [Max, MaxEl, Min, MinEl, Max-Min]).

tally_elements(Polymer) ->
    Tally = lists:foldl(fun tally_element/2, #{}, binary_to_list(Polymer)),
    [{MinEl, Min} | Rest] = lists:keysort(2, maps:to_list(Tally)),
    [{MaxEl, Max} | _] = lists:reverse(Rest),
    {{[MinEl], Min}, {[MaxEl], Max}}.

tally_element(Bin, Tally) ->
    maps:update_with(Bin, fun incr_1/1, 1, Tally).

incr_1(N) -> N+1.

steps(Template, _Insertions, 0) -> Template;
steps(Template, Insertions, StepsLeft) ->
    steps(step(Template, Insertions), Insertions, StepsLeft-1).

step(Template, Insertions) ->
    step(Template, Insertions, []).

step(<<End:1/binary>>, _Insertions, Polymer) ->
    list_to_binary(lists:reverse([End | Polymer]));
step(<<AB:2/binary, Rest/binary>>, Insertions, Polymer) ->
    C = maps:get(AB, Insertions),
    <<A:1/binary, B:1/binary>> = AB,
    step(<<B/binary, Rest/binary>>, Insertions, [C, A | Polymer]).
...
read_insertions([<<AB:2/binary, " -> ", C:1/binary>> | Pairs], Insertions) ->
    read_insertions(Pairs, Insertions#{AB => C}).
