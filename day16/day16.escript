...
%% The first step of decoding the message is to convert the
%% hexadecimal representation into binary. Each character of
...
%% representation of this packet might encode a few extra 0 bits at
%% the end; these are not part of the transmission and should be
%% ignored.
...
%% in any packet are represented as binary with the most significant
%% bit first. For example, a version encoded as the binary sequence
%% 100 represents the number 4.

%% Packets with type ID 4 represent a literal value. Literal value
%% packets encode a single binary number. To do this, the binary
%% number is padded with leading zeroes until its length is a multiple
%% of four bits, and then it is broken into groups of four bits. Each
%% group is prefixed by a 1 bit except the last group, which is
%% prefixed by a 0 bit. These groups of five bits immediately follow
%% the packet header. For example, the hexadecimal string D2FE28
%% becomes:
...
%% So, this packet represents a literal value with binary
%% representation 011111100101, which is 2021 in decimal.
...
%% subsequent binary data represents its sub-packets, an operator
%% packet can use one of two modes indicated by the bit immediately
...
%% Finally, after the length type ID bit and the 15-bit or 11-bit
%% field, the sub-packets appear.

%% For example, here is an operator packet (hexadecimal string
%% 38006F45291200) with length type ID 0 that contains two
%% sub-packets:
...
%%     that the length is a 15-bit number representing the number of
...
%% indicated in L (27) is reached, and so parsing of this packet
%% stops.
...
%% D2FE28 #{literal => 2021,type_id => 4,version => 6}

%% 38006F45291200 #{length_type_id => 0,sub_packet_length =>
%% 27,subpackets => [#{literal => 10,type_id => 4,version =>
%% 6},#{literal => 20,type_id => 4,version => 2}],type_id => 6,version
%% => 1}

%% EE00D40C823060 payload: #{length_type_id => 1,sub_packet_length =>
%% 3,subpackets => [#{literal => 1,type_id => 4,version =>
%% 2},#{literal => 2,type_id => 4,version => 4},#{literal => 3,type_id
%% => 4,version => 1}],type_id => 3,version => 7}
...
decode_literal(<<0:1, End:4, Rest/bitstring>>, Payload, Literal) ->
    BinStr = iolist_to_binary([string:pad(integer_to_list(Int, 2), 4, leading, $0)
                               || Int <- lists:reverse([End | Literal])
                              ]
                             ),
    {Payload#{literal => binary_to_integer(BinStr, 2)}
    ,Rest
    };
decode_literal(<<1:1, Bin:4, Rest/bitstring>>, Payload, Literal) ->
    decode_literal(Rest, Payload, [Bin | Literal]).

decode_operator(<<0:1, SubPacketBitLength:15, Rest/bitstring>>, Payload) ->
    <<Length:16>> = <<0:1, SubPacketBitLength:15>>,
    parse_subpackets_length(Length, Rest, Payload#{length_type_id => 0, sub_packet_length => Length});
decode_operator(<<1:1, SubPacketBitLength:11, Rest/bitstring>>, Payload) ->
    <<Length:16>> = <<0:5, SubPacketBitLength:11>>,
    parse_subpackets_count(Length, Rest, Payload#{length_type_id => 1, sub_packet_length => Length}).
...
parse_subpackets_length(<<>>, Subs) ->
    lists:reverse(Subs);
parse_subpackets_length(SubPackets, Subs) ->
    {Payload, Rest} = decode_packet(SubPackets),
    parse_subpackets_length(Rest, [Payload | Subs]).
...
parse_subpackets_by_count(0, Rest, Payloads) ->
    {lists:reverse(Payloads), Rest};
parse_subpackets_by_count(Count, Packet, Payloads) ->
...
read_input(File) ->
    ...
    [Hex] = [Line || Line <- binary:split(Lines, <<"\n">>, ['global']), Line =/= <<>>],
    << <<(binary_to_integer(H, 16))>> || <<H:2/binary>> <= Hex >>.
