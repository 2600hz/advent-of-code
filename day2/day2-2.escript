...
%% the program's values. A position in memory is called an address
...
main(_) ->
    ...
    {Noun, Verb} = search_for_output(Intcode, 19690720),
    io:format("input ~p / ~p: ~p~n", [Noun, Verb, Noun * 100 + Verb]).

search_for_output(Intcode, Answer) ->
    search_for_output(Intcode, Answer, 0, 0).

search_for_output(_Intcode, _Answer, 98, 99) ->
    throw({'error', 'search_space_exceeded'});
...
search_for_output(Intcode, Answer, Noun, Verb) ->
    try aoc:run_intcode(replace(Intcode, Noun, Verb)) of
        ...
        _:_ ->
            %% crashed Intcode programs
            search_for_output(Intcode, Answer, Noun+1, Verb)
    end.

replace(Intcode, Noun, Verb) ->
    Intcode#{1 => Noun
            ,2 => Verb
            }.
