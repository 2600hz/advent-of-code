...
count_valid_pw({FirstPos, SecondPos, Char, PW}, ValidCount) ->
    case is_valid_pw(FirstPos, SecondPos, Char, PW) of
        'true' -> ValidCount + 1;
        'false' -> ValidCount
    end.
...
line_to_pw(Line) ->
    {'match', [FirstPos, SecondPos, Char, PW]} = re:run(Line, <<"(\\d+)-(\\d+)\s+(\\w):\s+(.+)$">>, [{'capture', 'all_but_first', 'binary'}]),
    {binary_to_integer(FirstPos, 10), binary_to_integer(SecondPos, 10), Char, PW}.
