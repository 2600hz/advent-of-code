...
%%     15. Because your aim is 10, your depth increases by 2*10=20 to
%%     a total of 60.
...
main(_) ->
    Input = read_input("p2.txt"),
    p2_1(Input),
    p2_2(Input).

p2_1(Input) ->
    {Horizontal, Depth} = p2_1(Input, {0, 0}),
    io:format("horizontal: ~p depth: ~p product: ~p~n", [Horizontal, Depth, Horizontal * Depth]).

p2_1([], Position) -> Position;
p2_1([Step | Steps], Position) ->
    p2_1(Steps, step(Step, Position)).

step({'forward', F}, {Horizontal, Depth}) ->
    {Horizontal + F, Depth};
step({'up', U}, {Horizontal, Depth}) ->
    {Horizontal, Depth-U};
step({'down', D}, {Horizontal, Depth}) ->
    {Horizontal, Depth+D}.

p2_2(Input) ->
    {Horizontal, Depth, Aim} = p2_2(Input, {0, 0, 0}),
    io:format("horizontal: ~p depth: ~p aim: ~p product: ~p~n"
             ,[Horizontal, Depth, Aim, Horizontal * Depth]
             ).

p2_2([], Position) -> Position;
p2_2([Step | Steps], Position) ->
    p2_2(Steps, step2(Step, Position)).

step2({'down', D}, {Horizontal, Depth, Aim}) ->
    {Horizontal, Depth, Aim+D};
step2({'up', U}, {Horizontal, Depth, Aim}) ->
    {Horizontal, Depth, Aim-U};
step2({'forward', F}, {Horizontal, Depth, Aim}) ->
    {Horizontal + F, Depth + (Aim * F), Aim}.

read_input(File) ->
    {'ok', Lines} = file:read_file(File),
    [parse_line(Line) || Line <- binary:split(Lines, <<"\n">>, ['global']), Line =/= <<>>].

parse_line(<<"forward ", F/binary>>) ->
    {'forward', binary_to_integer(F, 10)};
parse_line(<<"down ", D/binary>>) ->
    {'down', binary_to_integer(D, 10)};
parse_line(<<"up ", U/binary>>) ->
    {'up', binary_to_integer(U, 10)}.
