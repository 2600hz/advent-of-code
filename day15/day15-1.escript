...
%%     had been spoken before, the next number to speak is the
...
%%     most recently on turns 5 and 2. So, the 6th number spoken is 5 -
%%     2, 3.
...
main(_) ->
    ...
    {Next, Game} = lists:foldl(fun(N, {I, G}) -> {I+1, G#{N => [I]}} end, {1, #{}}, Input),
...
play_game(Game, NthSpoken, TurnNo, PrevSpoken) ->
    %% io:format("turn ~p: prev spoken ~p: ~p~n", [TurnNo, PrevSpoken, Game]),
    case maps:get(PrevSpoken, Game, 'undefined') of
        'undefined' ->
            %% io:format("  ~p unspoken, saying 0~n", [PrevSpoken]),
            play_game(Game#{0 => [TurnNo]}, NthSpoken, TurnNo+1, 0);
        [Recent] when Recent =:= TurnNo-1 ->
            %% io:format("  ~p was said last turn, first time, saying 0~n", [PrevSpoken]),
            Spoken = maps:get(0, Game, []),
            play_game(Game#{0 => [TurnNo | Spoken]}, NthSpoken, TurnNo+1, 0);
        [RecentOne, RecentTwo |_] ->
            %% io:format("  ~p last spoken turn ~p, saying ~p~n", [PrevSpoken, RecentOne, RecentOne-RecentTwo]),
            Speaking = RecentOne - RecentTwo,
            Spoken = maps:get(Speaking, Game, []),
            play_game(Game#{Speaking => [TurnNo | Spoken]}, NthSpoken, TurnNo+1, Speaking)
    end.
