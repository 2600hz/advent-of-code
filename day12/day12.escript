...
main(_) ->
    Moons = read_moons(),
    Moved = step(1000, Moons),
    Energies = [energy(Moon) || Moon <- Moved],
    io:format("energies: ~w: ~p~n", [Energies, lists:sum(Energies)]).

energy(#moon{x=X, y=Y, z=Z
            ,dx=Dx, dy=Dy, dz=Dz
            }) ->
...
step(0, Moons) -> lists:keysort(#moon.name, Moons);
step(Step, Moons) ->
    step(Step-1, apply_velocity(apply_gravity(Moons))).

apply_velocity(Moons) ->
    [apply_moon_velocity(Moon) || Moon <- Moons].

apply_moon_velocity(#moon{x=X, dx=Dx
                         ,y=Y, dy=Dy
                         ,z=Z, dz=Dz
                         }=Moon
                   ) ->
...
apply_gravity([Moon], Gravitated) -> [Moon | Gravitated];
apply_gravity([Moon | Moons], Gravitated) ->
    {NewMoon, NewMoons} = lists:foldl(fun gravitate/2, {Moon, []}, Moons),
    apply_gravity(NewMoons, [NewMoon | Gravitated]).

gravitate(AMoon
         ,{TheMoon, NewMoons}
         ) ->
    {ANewMoon, TheNewMoon} = gravitate_moons(AMoon, TheMoon),
    {TheNewMoon, [ANewMoon | NewMoons]}.

gravitate_moons(#moon{x=Xa,y=Ya,z=Za}=MoonA
               ,#moon{x=Xb,y=Yb,z=Zb}=MoonB
               ) ->
...
test_input() ->
    ...
    <<"<x=-8, y=-10, z=0>\n"
      ...
      "<x=2, y=-7, z=3>\n"
      "<x=9, y=-8, z=-3>"
    >>. %% total energy after 100 steps: 1940
